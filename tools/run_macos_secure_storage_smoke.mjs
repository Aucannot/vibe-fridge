...
const checkScript = path.join(repoRoot, 'tools', 'check_secure_storage_smoke.mjs');

try {
  await main();
} catch (error) {
  console.error(`macOS secure storage smoke failed: ${error.message}`);
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    const status = await runNode([checkScript, vmServiceUrl]);
